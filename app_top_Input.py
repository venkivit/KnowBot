...
with st.sidebar:
    ...
    if uploaded_file:
        try:
            ...
            file_type = uploaded_file.type.split('/')[-1]  # Get file extension from MIME type
            ...
            if documents:
                if st.session_state.vector_store.add_documents(documents):
                    st.success(f"✅ Successfully added {len(documents)} documents from {uploaded_file.name}!")
                else:
    ...
    for doc, meta in zip(docs, metadata):
        with st.expander(f"Document {meta.get('source', 'Untitled')}"):
            ...
            try:
                formatted_time = pd.Timestamp(timestamp).strftime('%Y-%m-%d %H:%M') if timestamp else 'No date'
            ...
            if doc_source:
                ...
                if st.button("🗑️ Delete", key=delete_key):
                    if st.session_state.vector_store.delete_document(doc_source):
